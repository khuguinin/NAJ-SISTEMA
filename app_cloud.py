...
def inserir_processo(data_entrada, pasta, nome, telefone, descricao, assessor, prioridade, usuario):
    _exec(
        "INSERT INTO processos "
        "(data_entrada,numero_pasta,nome_assistido,telefone,descricao,assessor,prioridade,status,criado_por,criado_em) "
        "VALUES (%s,%s,%s,%s,%s,%s,%s,'Em Andamento',%s,%s)",
        (data_entrada, pasta, nome, telefone, descricao, assessor, prioridade, usuario, agora_str())
    )
def atualizar_processo(pid, data_entrada, pasta, nome, telefone, descricao, assessor, prioridade, status, usuario):
    _exec(
        "UPDATE processos SET data_entrada=%s,numero_pasta=%s,nome_assistido=%s,telefone=%s,descricao=%s,"
...
def tela_login():
    ...
    <div style="text-align:center;color:rgba(255,255,255,0.25);font-size:11px;margin-top:24px;">
        v4.0 &nbsp;•&nbsp; Python + Streamlit &nbsp;•&nbsp; Supabase
    </div>
    """, unsafe_allow_html=True)
# ══════════════════════════════════════════════════════════════════════════════
# HEADER
...
def render_sidebar() -> str:
    ...
    return menu.strip()
# ══════════════════════════════════════════════════════════════════════════════
# FORMULÁRIO DE PROCESSO (campo telefone adicionado aqui)
# ══════════════════════════════════════════════════════════════════════════════
def form_processo(key: str, permite_status: bool = False):
    ...
    with st.expander(label, expanded=True):
        ...
        with c1:
            ...
            # === CAMPO TELEFONE ADICIONADO SOMENTE AQUI (conforme solicitado) ===
            tel = st.text_input("📞 Telefone", value=fp.get("telefone",""),
                                key=f"{key}_tel", placeholder="(27) 99999-9999")
        with c2:
        ...
        with b1:
            if st.button("💾 Salvar", type="primary", key=f"{key}_save"):
                if not nome.strip():
                    ...
                    if fp["id"]:
                        atualizar_processo(fp["id"], dent.isoformat(), pasta, nome, tel, desc, av, prio, status, usuario)
                        st.success("✅ Processo atualizado com sucesso!")
                    else:
                        inserir_processo(dent.isoformat(), pasta, nome, tel, desc, av, prio, usuario)
            ...
            if _perfil == "assessor":
                if st.button("✏️", key=f"ed_{form_key}_{p['id']}", help="Editar"):
                    st.session_state[form_key] = {
                        ...
                        "nome_assistido": p["nome_assistido"], 
                        "telefone": p.get("telefone",""),          # ← necessário para edição funcionar
    ...
    if modo == "ativos":
        if st.button("➕ Cadastrar Novo Processo", type="primary", key=f"btn_novo_{modo}"):
            st.session_state[form_key] = {
                ...
                "nome_assistido": "", "descricao": "", "telefone":"",
